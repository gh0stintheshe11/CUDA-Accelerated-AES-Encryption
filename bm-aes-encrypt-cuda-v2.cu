...
/*
    ...
    Optimization:
        ...
        -v2 Coalesced Memory Access: In previous code, each thread is accessing
...
__global__ void aes_ctr_encrypt_kernel_v2(unsigned char *plaintext,
  ...
  int blocksPerThread =
      (numBlocks + gridDim.x * blockDim.x - 1) / (gridDim.x * blockDim.x);
  ...
  for (int block = 0; block < blocksPerThread; ++block) {
    ...
    if (blockId < numBlocks) {
      ...
      int blockSize =
          (blockId == numBlocks - 1 && dataSize % AES_BLOCK_SIZE != 0)
              ? dataSize % AES_BLOCK_SIZE
              : AES_BLOCK_SIZE;

      // Encrypt the counter to get the ciphertext block
      unsigned char ciphertextBlock[AES_BLOCK_SIZE];
      aes_encrypt_block(counter, ciphertextBlock, shared_expandedKey, d_sbox_v2);

      // XOR the plaintext with the ciphertext block
      for (int i = 0; i < blockSize; ++i) {
        ciphertext[blockId * AES_BLOCK_SIZE + i] =
            plaintext[blockId * AES_BLOCK_SIZE + i] ^ ciphertextBlock[i];
      }
    }
  }
}
...
std::pair<double, double> aes_encrypt_cuda_v2(unsigned char *plaintext,
  ...
  aes_ctr_encrypt_kernel_v2<<<blocksPerGrid, threadsPerBlock>>>(
