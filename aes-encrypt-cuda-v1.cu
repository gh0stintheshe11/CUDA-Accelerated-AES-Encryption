...
__constant__ unsigned char d_rcon_v1[11];

// Declare constant memory variables for the IV and expanded key
__constant__ unsigned char constantIv_v1[AES_BLOCK_SIZE];
__constant__ unsigned char constantExpandedKey_v1[176];

__global__ void aes_ctr_encrypt_kernel_v1(unsigned char *plaintext, unsigned char *ciphertext, int numBlocks) {
    ...
    if (threadIdx.x < AES_BLOCK_SIZE) {
        sharedIv[threadIdx.x] = constantIv_v1[threadIdx.x];
    }
    ...
    if (tid < numBlocks) {
        ...
        aes_encrypt_block(localIv, block, constantExpandedKey_v1, d_sbox_v1);  // Use constantExpandedKey_v1 here
...
double aes_encrypt_cuda_v1(unsigned char *plaintext, size_t dataSize,
  ...
  cudaMemcpyToSymbol(d_rcon_v1, h_rcon, sizeof(h_rcon));
  ...
  copyToConstantMemory(constantIv_v1, iv, constantExpandedKey_v1, expandedKey);
  ...
  for (int i = 0; i < numStreams; ++i) {
    ...
    if (startBlock < endBlock) {
      ...
      aes_ctr_encrypt_kernel_v1<<<blocks, threads, 0, streams[i]>>>(
